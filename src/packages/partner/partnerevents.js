...
function PartnerEvents({ items = [], open = false, toggle = () => {}, onEventClick = null }) {
  ...
  const filteredItems = useMemo(() => {
    ...
    if (!showOld) {
      list = list.filter((item) => {
        ...
        if (!dt) return true; // keep if no date provided
        const d = new Date(dt);
      ...
      <div className="pd-section__body">
        ...
        {hasItems ? (
          (filteredItems && filteredItems.length > 0) ? (
            ...
            <div className="pagePartnerDetail__placeholder">No events match your search.</div>
          )
        ) : (
