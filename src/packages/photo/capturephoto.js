...
const CapturePhoto = ({ show, onPhoto, onQRCode, onClose, onId, useRearCamera = true }) => {
  const [photo, setPhoto] = useState(null);
  const [error, setError] = useState("");
  const [input, setInput] = useState(21);
  const [cameraFacingMode, setCameraFacingMode] = useState(useRearCamera ? "environment" : "user");
